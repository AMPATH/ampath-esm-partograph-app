...
const validationSchema = z.object({
    ...
    foetalHeartRate: z.string({ required_error: "Heart rate is required" })
});

type PostFormSchema = z.infer<typeof validationSchema>;

interface FoetalHeartRateFormProps {
    mutated: () => void
}

const FoetalHeartRateForm: React.FC<Workspace2DefinitionProps<FoetalHeartRateFormProps>> = ({ workspaceProps: { mutated }, closeWorkspace }) => {
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                <TextInput
                                    {...field}
                                    id="foetalHeartRate"
                                    labelText={t("foetalHeartRate", "Foetal heart rate *")}
                                    placeholder={t('enterFoetalHeartRate', 'Enter foetal heart rate')}
                                    invalid={!!errors.foetalHeartRate}
                                    invalidText={errors.foetalHeartRate?.message}
                                />
                            )}
                        />
                    </ResponsiveWrapper>
...
export default FoetalHeartRateForm;